...
class TelemetrySimulatorProvider:
    ...
    def read_data(self) -> dict:
        ...
        return {
            ...
            "sinyal_gücü":       round(random.uniform(-105, -85), 1),
            "paket_no":          int(t * 10),
            "paket_kaybi":       round(random.uniform(0, 2), 1),
            "vertical_velocity": round(vel * math.sin(math.radians(85)) + random.gauss(0, 1.0), 1),
            "gyro_x":            round(random.gauss(0, 5), 2),
            "gyro_y":            round(random.gauss(0, 5), 2),
            "gyro_z":            round(random.gauss(0, 5), 2),
